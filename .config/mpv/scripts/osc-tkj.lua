...
--[[
    seekbar_thumbnails.lua 0.4.2 - commit a2de250 (branch master)
    https://github.com/TheAMM/seekbar_thumbnails
...
function join_paths(...)
  ...
  --sep = '/' -- fallegra að nota þetta bara 
  local result = "";
  for i, p in pairs({...}) do
    if p ~= "" then
...
-- Af einhverjum ömurlegum ástæðum þarf þetta script að nota \\ 
-- hefur sitt eigið join_paths fall sem þarf að nota
...
if ON_WINDOWS then 
    ...
    thumbnails_dir = mp.command_native({"expand-path", "~/mpv_thumbs/thumbnails_seekbar/"})
end
local thumbnailer_options = {
    -- The thumbnail directory
    cache_directory = thumbnails_dir,--join_paths(default_cache_base, "mpv_thumbs_cache"),
...
read_options(thumbnailer_options, "seekbar_thumbnails")
local Thumbnailer = {
...
function Thumbnailer:on_thumb_progress(index)
    if type(self.state.thumbnails[index]) == 'number' then     -- ég bætti við þessu if til að laga bug (virkaði ekki að draga nýtt video)
...
function Thumbnailer:register_client()
    ...
    mp.register_script_message("seekbar_thumbnails-ready", function(index, path)
        self:on_thumb_ready(tonumber(index), path)
    end)
    mp.register_script_message("seekbar_thumbnails-progress", function(index, path)
        self:on_thumb_progress(tonumber(index), path)
    end)

    mp.register_script_message("seekbar_thumbnails-worker", function(worker_name)
        if not self.workers[worker_name] then
            msg.debug("Registered worker", worker_name)
            self.workers[worker_name] = true
            mp.commandv("script-message-to", worker_name, "seekbar_thumbnails-slaved")
        end
    end)
    ...
    mp.observe_property("video-dec-params", "native", function()
        ...
        if self.state.available and thumbnailer_options.autogenerate then
            ...
            if type(duration) == 'number' then    -- ég bætti við þessu if til að laga bug (virkaði ekki að draga nýtt video)
...
function Thumbnailer:prepare_source_path()
    ...
    if self.state.is_remote and thumbnailer_options.remote_direct_stream then
        ...
        -- TKJ ATH Ég bætti við eftirfarandi villu, skrítið bug:
        -- Þetta sækir eitthvað ógeðslega langt stream path (getur slökkt á þessu í 
        -- seekbar_thumbnail_settings.conf með remote_direct_stream=no en 
        -- af einhverjum ástæðum virkaði þetta heldur ekki með þetta off)
        -- Aftast í þetta stream path kom af einhverjum ástæðum ytdl_description= ... 
        -- og svo allt youtube descriptionið og það fokkaði öllu upp,
        -- lagaðist við að sleppa bara því
        file_path = Split(file_path, ",ytdl_description=")[1] 
        ...
        elseif #file_path > 1024 then
            ...
            --msg.warn("Using playlist workaround due to long source path") ég commentaði út því nenntiggi hafa
        end
    end
    ...
    if worker_count == 0 then
        ...
        for worker_name, worker_frames in pairs(worker_jobs) do
            if #worker_frames > 0 then
                ...
                mp.commandv("script-message-to", worker_name, "seekbar_thumbnails-job", state_json_string, frames_json_string)
            end
        end
    end
end
...
Sections are denoted with -- seekbar_thumbnails.lua --
...
local user_opts = {
    ...
    valign = 1.0,               -- vertical alignment, -1 (top) to 1 (bottom)
    ...
    seekrangestyle = "bar",     -- bar, line, slider, inverted or none (Seekable ranges = cached)
    seekrangeseparate = true,   -- wether the seekranges overlay on the bar-style seekbar
    seekrangealpha = 80,        -- transparency of seekranges
...
--
-- TKJ Options sem ég bætti við:
--
    --osc_peek_timer = 5, -- of flókið, betra að ntoa bara hidetimeout
    seekbar_height = 66,
    seekbar_visible = true,
    compact_audiomode = false,
    big_osc = 'auto',
    persist_tooltip = 'none', -- opts: none, without_chap, with_chap
    highlight_chapters = true,

}
if not ON_WINDOWS then 
    user_opts.vidscale = false -- reynslulögmál
end
...
function display_thumbnail(pos, value, ass)
    ...
    if not ((duration == nil) or (value == nil)) then
        ...
        if big_osc_active() then 
            vertical_offset = vertical_offset + 8
        end
        if (mp.get_property_number('chapter-list/count') > 1) and user_opts.highlight_chapters then -- make room for chapter marks
            vertical_offset = vertical_offset + osc_fs - 2
        end
...
function find_chapter_index_from_time(time_list, timestamp)
    -- time_list is an array of timestamps
    -- function returns the index of the last 
    -- timestamp before input timestamp
    -- (ie the index of the current chapter wrt input timestamp)
    local index = 0
    for ind, ts in pairs(time_list) do 
        if not (ts < timestamp) then
            return ind - 1
        end
        index = ind
    end
...
function display_hovered_chapter(pos, value, ass, slider_lo, alpha_string, element, no_thumb)
    -- TKJ modelað eftir display_thumbnail

    -- If there are no (or one) chapters, bail
    chapcount = mp.get_property_number('chapter-list/count')
    if (chapcount < 2) then
        return
    end
    ...
    -- Make titles and start times tables
    i = 0
    ...
    -- Make start and end times table
    local chapter_borders = {}
    local last_s
    if times[0] == 0 then -- þurfum 2 tilfelli því næ í x-hnit frá marker nibbles sem birtast ekki á jaðrinum
        last_s = 0
    else
        last_s = nil 
    end
    ...
    if not (element.slider.markerF == nil) and (slider_lo.gap > 0) then
        ...
        for _,marker in pairs(markers) do
            if ( element.slider.min.value < marker) and (marker < element.slider.max.value) then
                local s = get_slider_ele_pos_for(element, marker)
                if last_s == nil then goto continue end
                chapter_borders[i] = {c_start=last_s, c_end=s}
                i = i + 1
                ::continue:: 
                last_s = s  
            end
        end
        chapter_borders[i] = {c_start=last_s, c_end=get_slider_ele_pos_for(element, element.slider.max.value)}
    end
    ...
    if not ((duration == nil) or (value == nil)) then
        ...
        idx = find_chapter_index_from_time(times, target_position)


        -- Gotta know if displaying thumb progress
        local thumbs_ready = Thumbnailer.state.finished_thumbnails
        local thumbs_total = Thumbnailer.state.thumbnail_count
        local display_progress = thumbs_ready ~= thumbs_total and not thumbnailer_options.hide_progress

        local chapter_marker_height = osc_fs + 1
        -- if no_thumb == true then 
        --     chapter_marker_height = osc_fs + 1
        -- else
        --     chapter_marker_height = Thumbnailer.state.thumbnail_size.h + 28
        --     if display_progress then 
        --         chapter_marker_height = chapter_marker_height + 30
        --     end    
        -- end
        if big_osc_active() then 
            chapter_marker_height = chapter_marker_height + 7
        end
        local chapter_marker_font_color = '9e9e9e'
        local chapter_marker_font_alpha = '00' -- í hexa svo td 00 eða 24 eða hh eða a3
        if titles[idx] == nil then 
            chapter_title = ''
        else
            chapter_title = titles[idx]
        end 


        ass:new_event()
        ass:pos(pos.x, pos.y - chapter_marker_height)
        ass:an(2)
        ass:append(slider_lo.tooltip_style)
        ass_append_alpha(ass, slider_lo.alpha, 0)
        ass:append(alpha_string)
        ass:append(chapter_title)

        -----------------------------------------------------------------------------------
        -- Highlight hovered chapter
        local elem_geo = element.layout.geometry
        local diamond_color = rgb_to_bgr('044359') --'5c4507'
        local diamond_style = "{\\blur0\\bord0\\1c&H" .. diamond_color .. "\\3c&H" .. diamond_color .. "\\fs20}"
        local sa_split, static_ass_without_chapters
        -- Af einhverjum ástæðum þarf ég byrjunina á static_ass en ekki kaflapartinn 
        -- mjög hacky leið hér til að sækja bara fyrri hlutan
        sa_split = Split(element.static_ass.text, 'm')
        static_ass_without_chapters =  sa_split[1] .. 'm' .. sa_split[2] .. 'm' .. sa_split[3] 

        ass:append("{}") -- hack to troll new_event into inserting a \n
        ass:new_event()
        ass:pos(elem_geo.x, elem_geo.y)
        ass:an(elem_geo.an)
        ass:append(element.layout.style)
        ass:append(diamond_style)
        ass:append(static_ass_without_chapters)

        local borders = chapter_borders[idx]
        if chapter_borders[idx] ~= nil then 
            local a = chapter_borders[idx]['c_start']
            local b = chapter_borders[idx]['c_end']
            ass:rect_cw(a, slider_lo.border, b, elem_geo.h - slider_lo.border)
        end
        -----------------------------------------------------------------------------------
...
-- TKJ
-- Nokkrir (öfugir) hex kóðar (frá ljósari í dekkri):
-- (sóttir héðan: https://htmlcolorcodes.com/)
-- Gulur:               e7f9fe, cff3fc, 9fe7f9, 6fdcf7, 3fd0f4
-- Appelsínugulur I:    e6eefb, ccddf6, 99bbed, 6698e5, 3376dc
-- Appelsínugulur II:                           7ab2f0, 4e98eb, 227ee6 
-- Grænn:               eff7e9, dfefd4, bfdfa9, a0ce7d, 80be52
-- Blár I:              fbf5eb, f8ead6, f1d6ae, e9c185, e2ad5d
-- Blár II:             f8f2ea, f1e6d4, e3cca9, d5b37f, c79954
-- Fjólublár:           f7ecf4, efdae8, deb4d2, ce8fbb, bd69a5
-- Rauður:              eaebf9, d5d7f2, aab0e6, 8088d9, 5561cd

local ows = '9e9e9e' --'ababab' -- 'DCDCDC'  -- originally white stuff 
local obs = '000000'  -- originally black stuff
local seekbarborder_color = '000000'
local seekbarcore_color   = rgb_to_bgr('003447') --'7a7a00'--'454400' --'444500' --'000066' --'000099'
-- local seekbarcore_color   = rgb_to_bgr('711313') --'7a7a00'--'454400' --'444500' --'000066' --'000099'
local x_button_color      = '3232a2' --'2222B2' #a23232  #9a3a3a
local title_opac = 0

local osc_styles = {
    bigButtons = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs50\\fnmpv-osd-symbols}",
    smallButtonsL = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs19\\fnmpv-osd-symbols}",
    smallButtonsLlabel = "{\\fscx105\\fscy105\\fn" .. mp.get_property("options/osd-font") .. "}",
    smallButtonsR = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs30\\fnmpv-osd-symbols}",
    topButtons = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs12\\fnmpv-osd-symbols}",

    elementDown = "{\\1c&H999999}",
    timecodes = "{\\blur0\\bord0\\1c&H" .. seekbarcore_color .. "\\3c&H" .. seekbarcore_color .. "\\fs20}",  -- barinn í seekbar
    vidtitle = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs12\\q2}",
    box = "{\\rDefault\\blur0\\bord1\\1c&H" .. obs .. "\\3c&H" .. seekbarborder_color .. "}", -- seinni hér er borderinn á seekbar

    topButtonsBar = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs18\\fnmpv-osd-symbols}",
    smallButtonsBar = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs28\\fnmpv-osd-symbols}",
    timecodesBar = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs27}",
    timePosBar = "{\\blur0\\bord".. user_opts.tooltipborder .. "\\1c&H" .. ows .. "\\3c&H" .. obs .. "\\fs30}",
    vidtitleBar = "{\\blur0\\bord0\\1c&H" .. ows .. "\\3c&H" .. ows .. "\\fs18\\q2}",
    
    wcButtons  = "{\\1c&H" .. ows .. "\\fs" .. osc_fs .. "\\fnmpv-osd-symbols}", --dótið eftir \\ nr 2 virðist vera font, defaultið var fs24
    wcButtons2 = "{\\1c&H" .. x_button_color .. "\\fs" .. osc_fs .. "\\fnmpv-osd-symbols}", -- til að hafa annan lit
    wcTitle = "{\\1c&H" .. ows .. "\\fs" .. osc_fs .. "\\q2}",                  --sama hér
    wcBar = "{\\1c&H" .. obs .. "}",
}
                ...
                for _,marker in pairs(markers) do
                    if (marker > element.slider.min.value) and
                        ...
                        if (slider_lo.gap > 1) then -- draw triangles
                            ...
                            --top
                            if (slider_lo.nibbles_top) then -- tkj stækkaði chapter markerana hér
                                -- static_ass:move_to(s - (a/2), slider_lo.border)
                                -- static_ass:line_to(s + (a/2), slider_lo.border)
                                -- static_ass:line_to(s, foV)
                                static_ass:move_to(s - a*0.65, slider_lo.border)
                                static_ass:line_to(s + a*0.65, slider_lo.border)
                                static_ass:line_to(s, foV*6)
                            end
                            ...
                            if (slider_lo.nibbles_bottom) then
                                ...
                                static_ass:move_to(s - a*0.65, elem_geo.h - slider_lo.border)
                                static_ass:line_to(s, elem_geo.h - foV*6)
                                static_ass:line_to(s + a*0.65, elem_geo.h - slider_lo.border)
                            end
                            ...
                            if (slider_lo.nibbles_top) then
                                static_ass:rect_cw(s - 3, slider_lo.border,
                                    s + 3, slider_lo.border + slider_lo.gap + 10);
                            end
...
function render_elements(master_ass)
    ...
    for n=1, #elements do
        ...
        if (element.type == "slider") then
            ...
            -- draw pos marker
            -- TKJ: 
            -- foH er byrjunin á barnum
            -- pos er bara timestamp í sek
            -- xp er staðsetning á elementi (td tígul) fyrir núverandi pos
            local foH, xp
            local pos = element.slider.posF()
            local foV = slider_lo.border + slider_lo.gap
            local innerH = elem_geo.h - (2 * foV)
            local seekRanges = element.slider.seekRangesF()
            local seekRangeLineHeight = innerH --/ 5  Hæðin á cache merkinu
            ...
            if pos then
                ...
                xp = get_slider_ele_pos_for(element, pos) + 2
                ...
                if slider_lo.stype ~= "bar" then
                    ...
                    local r = 2.2*((user_opts.seekbarhandlesize * innerH) / 2)
                    local diamond_color = rgb_to_bgr('074961') -- blai: 074961, raudi: 981b1b, '5c4507' 
                    local diamond_style = "{\\blur0\\bord0\\1c&H" .. diamond_color .. "\\3c&H" .. diamond_color .. "\\fs20}"
                    local sa_split, static_ass_without_chapters
                    -- Af einhverjum ástæðum þarf ég byrjunina á static_ass en ekki kaflapartinn 
                    -- mjög hacky leið hér til að sækja bara fyrri hlutan
                    ...
                    tkj_diamond = assdraw.ass_new()
                    tkj_diamond:merge(style_ass)
                    tkj_diamond:append(diamond_style)
                    tkj_diamond:append(static_ass_without_chapters)
                    -- tkj_diamond:draw_start() -- {p4} dótið sem byrjar draw er innifalið í static_ass 
                    tkj_diamond:hexagon_cw(
                        xp - r + 2, 40 , 
                        xp + r + 2, 40 + 2*r, 
                        r, nil)
                    tkj_diamond:move_to(0,0)
                    tkj_diamond:draw_stop()
                    tkj_draw_diamond = true
                    


                    elem_ass:rect_cw(foH, foV, xp, elem_geo.h) -- TKJ Munurinn á hæð seekbar og cache fékkst með því að draga "- foV - h*0.5" frá elem_geo.h hér
                    ...
                    if seekRanges and not user_opts.seekrangeseparate and slider_lo.rtype ~= "inverted" then
                        ...
                        for _,range in pairs(seekRanges) do
                            if range["start"] < pos then
                                ...
                                elem_ass:rect_ccw(
                                    pstart, elem_geo.h - foV - seekRangeLineHeight, 
                                    pend, elem_geo.h - foV)
                            end
                        end
                    end
            ...
            if not (element.slider.tooltipF == nil) then
                ...
                if mouse_hit(element) then
                    ...
                    local tooltip_height_shift = 5
                    if big_osc_active() then 
                        tooltip_height_shift = tooltip_height_shift + 3
                    end

                    -- tooltip label
                    elem_ass:new_event()
                    elem_ass:pos(tx, ty - tooltip_height_shift)  -- HÆÐIN Á SEEKBAR MOUSEOVER TIMECODE ER HÉR TKJ
                    elem_ass:an(an)
                    ...
                    local alpha_string = string.format(
                        "{\\1a&H%X&\\2a&H%X&\\3a&H%X&\\4a&H%X&}",
                        ar[1], ar[2], ar[3], ar[4]
                    )
                    elem_ass:append(alpha_string)
                    ...
                    -- TKJ Birta hoveraða kaflann: modelað eftir display_thumbail hér beint fyrir ofan
                    if user_opts.highlight_chapters then 
                        display_hovered_chapter(
                            {x=get_virt_mouse_pos(), y=ty, a=an}, 
                            sliderpos, 
                            elem_ass, 
                            slider_lo,      -- required for ass_style
                            alpha_string,   -- required for ass_style
                            element,
                            false           -- without_thumbnail (hliðrun)
                        )
                    end


                elseif (user_opts.persist_tooltip ~= 'none') and (mp.get_property('percent-pos') ~= nil) then  -- tkj bætti við fyrir toggle_tooltip
                    local sliderpos = mp.get_property('percent-pos')
                    local tooltiplabel = element.slider.tooltipF(sliderpos)
                    ...
                    local tx = element.hitbox.x1 + (element.hitbox.x2 - element.hitbox.x1)*sliderpos/100
                    ...
                    if user_opts.persist_tooltip == 'with_chap' then
                        -- TKJ Birta hoveraða kaflann: modelað eftir display_thumbail hér beint fyrir ofan
                        display_hovered_chapter(
                            {x=tx, y=ty, a=an}, 
                            sliderpos, 
                            ...
                            alpha_string,   -- required for ass_style
                            element,
                            true            -- without_thumbnail (hliðrun)
                        )    
                    end
                end
            end
        ...
        if tkj_draw_diamond then 
            master_ass:merge(tkj_diamond)
        end
...
function window_controls(topbar)
    local wc_geo= {
        x = 0,
        y = osc_param.playresy - 36,-- 30 + user_opts.barmargin,  -- TKJ Þykkt og staðsetning titlebars er hér (font stillt sér uppi)
        an = 1,
        w = osc_param.playresx,        -- og hér
        h = 30,
    }
    if big_osc_active() then 
        wc_geo['y'] = osc_param.playresy --30 + user_opts.barmargin + 20
        wc_geo['h'] = 30 + 24
    end
    ...
    lo.alpha[1] = 255--user_opts.boxalpha
    ...
    -- TKJ Faldi hér stórt chunk til að drepa minimize maximize og close
    --    -- Close: 🗙
    ...
    --    lo.style = osc_styles.wcButtons2
    --
    --    -- Minimize: 🗕
    ...
    ne.content = function ()
        ...
        title = title:gsub("\\n", " "):gsub("\\$", ""):gsub("{","\\{")
        if type(title) == 'string' then -- Veit ekki af hverju en það virkar ekki að setja border í lo.style
            local title_border_size
            if big_osc_active() then 
                title_border_size = 1 
            else
                title_border_size = 1
            end
            title = "{\\3c&H000000\\bord" .. title_border_size .. "\\blur0.4\\3a&H00&}" .. title
        end
    ...
    lo.layer = 51 -- tkj til að hafa titilinn yfir litaða seekbar

    if big_osc_active() then 
        lo.geometry = {
            x = osc_param.playresx*0.5, y = wc_geo.y - 2,  -- TKJ breytti hér til að miðja titil videosins 
            an = 2, w = titlebox_w, h = wc_geo.h 
        }
        lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}",
            "{\\1c&H" .. ows .. "\\fs" .. osc_fs + 16 .. "",
            titlebox_left + left_pad, wc_geo.y - wc_geo.h,
            titlebox_right - right_pad , wc_geo.y + wc_geo.h)
    else
        lo.geometry = { 
            x = osc_param.playresx*0.5, y = wc_geo.y,  -- TKJ breytti hér til að miðja titil videosins
            an=2, w = titlebox_w, h = wc_geo.h 
        }
        lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}",
            "{\\1c&H" .. ows .. "&\\fs" .. osc_fs .. "}",
    ...
    lo.alpha[1] = 0     -- TKJ HÉR ER OPACITY Á TITLE TITLI VIDEOS

    add_area(   
        "window-controls-title", titlebox_left, 
        0, titlebox_right, wc_geo.h
    )
end
...
-- slim box layout (eina sem ég nota)
layouts["slimbox"] = function ()

    local osd_w, osd_h = mp.get_osd_size() -- mín breyting
    local breidd

    if type(osd_w) == 'number' then
        breidd = math.min(osd_w, 2800) - 100
    else 
        breidd = 2800
    end

    -- Sýnist tc_w ráða breidd svæðisins sem er hægt að klikká til að breyta timecode formatti
    -- seekbar þykktin er ele_h (default 20) (þetta var neðar en færði því þurfti ele_h fyrr
    local tc_w, ele_h, inner_w = 100, osc_fs + 3, breidd
    local osc_geo = {
        w = breidd,   -- width  TKJ
        h = user_opts.seekbar_height,     -- height
        r = 10,     -- corner-radius
    }
    ...
    -- add_area gerir...
    -- get_hitbox_coords tekur in: x, y, an, w, h 
    --                  og skilar: x0, y0, x1, y1 ferhyrning fyrir add_area
    -- an er alignment og ræður því í hvaða átt út frá x, y ferhyrningurinn kemur (td an=1 lætur x,y vera efra vinstra hornið
    -- an=2 lætur x,y vera miðpunkt efri hliðar, an=3 lætur x,y vera efra hægra hornið, an=4 lætur x,y vera miðpunkt vinstri 
    -- hliðar, etc... sem þýðir að an=5 (sem er default hér) lætur x,y vera miðju ferningsins
    ...
    if big_osc_active() then 
        sh_area_y0 = osc_param.playresy - 2*ele_h -- show/hide are, settu bara ele_h hér ef þú villt sýna seekbar sama hvar mús hreyfist
        sh_area_y1 = osc_param.playresy
        add_area("input", 0, osc_param.playresy-ele_h, osc_param.playresx, osc_param.playresy)  -- TKJ Hér er skilgreint hvaða ferhyrningur dregur ekki gluggan
    else
        add_area("input", get_hitbox_coords(posX, posY-ele_h/2, 5, osc_geo.w, ele_h))  -- TKJ Hér er skilgreint hvaða ferhyrningur dregur ekki gluggan
    end
    ...
    -- local tc_w, ele_h, inner_w = 100, 20, osc_geo.w - 100  -- Þetta var hér en ég færði upp beint fyrir ofan osc_geo því þurfti ele_h fyrr

    -- styles 
    local styles 
    if big_osc_active() then 
        local timecode_font_size = osc_fs + 16 - 1
        local timecode_border = 1.0
        styles = {
            box = "{\\rDefault\\blur0\\bord1\\1c&H" .. obs .. "\\3c&H" .. ows .. "}",
            timecodes = "{\\1c&H" .. ows .. "\\3c&H" .. obs .. "\\fs" .. timecode_font_size .. "\\bord" .. timecode_border .. "\\blur0.4}", -- Letrið í timecode undir seekbar til hliða TKJ 
            tooltip = "{\\1c&H" .. ows .. "\\3c&H" .. obs .. "\\fs23\\bord1.6\\blur0.5}", -- Letrið í tooltip timecode yfir seekbar
        }
    else
        local timecode_font_size = osc_fs
        local timecode_border = 1.0
        styles = {
            box = "{\\rDefault\\blur0\\bord1\\1c&H" .. obs .. "\\3c&H" .. ows .. "}",
            timecodes = "{\\1c&H" .. ows .. "\\3c&H" .. obs .. "\\fs" .. timecode_font_size .. "\\bord" .. timecode_border .. "\\blur0.4}", -- Letrið í timecode undir seekbar til hliða TKJ 
            tooltip = "{\\1c&H" .. ows .. "\\3c&H" .. obs .. "\\fs20\\bord1.6\\blur0.5}", -- Letrið í tooltip timecode yfir seekbar
        }
    end
    ...
    if mp.get_property('cache') == 'yes' then 
        lo.alpha[1] = 100  --TKJ SEEKBAR ALPHA/TRANSPARENCY/OPACITY
    else
        lo.alpha[1] = 0
    end
    ...
    -- Timecodes
    --
    lo = add_layout('pause_indicator')
    if big_osc_active() then 
        lo.geometry =
            {x = osc_param.playresx*0.5, y = posY - 2.2*ele_h,
            an = 7, w = tc_w, h = ele_h}
    else
        lo.geometry =
            {x = osc_param.playresx*0.5, y = posY + 1,
            an = 7, w = tc_w, h = ele_h}
    end
    ...
    if big_osc_active() then 
        lo.geometry =
            {x = posX - (inner_w/2) + osc_geo.r, y = posY - ele_h + 2, -- hliðra hér vinstri tímanum nær brúninni TKJ
            an = 7, w = tc_w, h = ele_h}
    else
        lo.geometry =
            {x = posX - (inner_w/2) + osc_geo.r + 38 , y = osc_param.playresy - 36,--posY + 2, -- hliðra hér vinstri tímanum nær brúninni TKJ
            an = 2, w = tc_w, h = ele_h}
    end
    lo.style = styles.timecodes
    lo.alpha[3] = 0 --user_opts.boxalpha
    lo.layer = 51 -- default er 50, stækkaði til að setja yfir seekbar

    lo = add_layout("tc_right")
    if big_osc_active() then 
        lo.geometry =
            {x = posX + (inner_w/2) - osc_geo.r, y = posY - ele_h + 2, -- hliðra hér hægri tímanum nær brúninni
            an = 9, w = tc_w, h = ele_h}
    else
        lo.geometry =
            {x = posX + (inner_w/2) - osc_geo.r - 38, y = osc_param.playresy - 36,--posY + 2, -- hliðra hér hægri tímanum nær brúninni
            an = 2, w = tc_w, h = ele_h}
    end
    lo.style = styles.timecodes
    lo.alpha[3] = 0 --user_opts.boxalpha
    ...
    -- Cache
    cache_color = '009900'
    cache_style = "{\\blur0\\bord0\\1c&H" .. cache_color .. "\\3c&H" .. cache_color .. "\\fs20}"
...
--    end
--    add_area("showhide", 0, sh_area_y0, osc_param.playresx, sh_area_y1+1)
--
--    local lo, geo
--
--    -- Background bar
...
function osc_init()
    ...
    -- tkj pause symbol element
    ne = new_element("pause_indicator", "button")
    ...
    ne.content = function ()
        ...
        return ''
        --return "Cache: " .. (min > 0 and              -- Þetta á að sýna hve mikið cache er loadað í streaming videoum TKJ
    ...
    layouts[user_opts.layout]()
        ...
        if true then -- TKJ á linux var þetta að fela titilinn, hjö á hnútinn
...
function mouse_leave()
    if get_hidetimeout() >= 0 then
        --hide_osc()                -- TKJ ég commentaði þetta út sem slekkur alveg á þessu falli
        -- Ef þetta er virkt hverfur OSC alltaf þegar músinn fer  úr glugganum eða fer af OSC-inu
        -- (þó always visible sé í gangi)
    end
...
function visibility_mode(mode, no_osd)
    ...
    if not no_osd and tonumber(mp.get_property("osd-level")) >= 1 then
        --mp.osd_message("OSC visibility: " .. mode)    ÉG COMMENTAÐI ÞETTA ÚT TKJ
    end
...
-----------------------------------------------------------------
------------------------------ TKJ ------------------------------
local function cycle_filetitle_mediatitle()
    if user_opts.title == '${media-title}' then 
        user_opts.title = '${filename/no-ext}'
    else
        user_opts.title = '${media-title}'
    end
end
mp.add_key_binding(nil, 'cycle_filetitle_mediatitle', cycle_filetitle_mediatitle)

mp.register_event("file-loaded", function()
    -- Set title to media-title if oneline video
    -- else, just use the filename
    local fullpath = mp.get_property('path')
    if (fullpath:sub(1, 7) == 'ytdl://') or(fullpath:sub(1, 8) == 'https://') or (fullpath:match('youtube') ~= nil) or (fullpath:match('Vefjarvideo') ~= nil) then 
        user_opts.title = "${media-title}"
    else
        user_opts.title = "${filename/no-ext}"
    end
...
mp.register_event("file-loaded", function()
    -- Sets OSC='always' on start for audio-only files
    local novideo = true
    local x = mp.get_property_native('track-list')
    for i, track in pairs(x) do
        --print(track['type'])
...
local function cycle_osc_auto_always()
    -- Cycles OSC visibility modes, skipping "never"
    if user_opts.visibility == 'never' then 
        user_opts.visibility = 'always'
    elseif user_opts.visibility == 'auto' then
        user_opts.visibility = 'always' 
    else -- this is if user_opts.visibility == 'always' 
        user_opts.visibility = 'auto'
    end
    visibility_mode(user_opts.visibility, true)
end
mp.add_key_binding(nil, 'cycle-osc-auto-always', cycle_osc_auto_always)

local function peek_osc()
    -- If the OSC vis is set to auto, show it briefly
...
mp.register_event('file-loaded', peek_osc)

-----------------------------------------------------------------------------
------------------------------ Hide seekbar ---------------------------------
local function toggle_seekbar()
    if user_opts.seekbar_visible then 
        user_opts.seekbar_height = -50
        user_opts.seekbar_visible = false
        mp.osd_message('Seekbar off')
    else
        user_opts.seekbar_height = 66
        user_opts.seekbar_visible = true
        mp.osd_message('Seekbar on')

    end
end
mp.add_key_binding(nil, 'toggle-seekbar', toggle_seekbar)


-----------------------------------------------------------------------------
------------------------------ Audio mode ---------------------------------
local function toggle_compact_audio()
    if compact_audiomode then 
        mp.set_property("geometry", "100%:100%")
        compact_audiomode = false
        mp.osd_message('Compact Audio Mode Off')
    else
        mp.set_property("geometry", "20x1000")
        compact_audiomode = true
        mp.osd_message('Compact Audio Mode On')
    end
end
mp.add_key_binding(nil, 'toggle-compact-audio', toggle_compact_audio)

-----------------------------------------------------------------------------
-------------------------------- Big OSC ------------------------------------
-- TKJ fall til að tjékká big_osc
function big_osc_active()
    if (user_opts.big_osc == 'auto') and state.fullscreen then 
        return true 
    elseif user_opts.big_osc == true then 
        return true 
    else
        return false 
    end
...
local function toggle_big_osc()
    if user_opts.big_osc == 'auto' then 
        user_opts.big_osc = true 
        mp.osd_message("OSC size: always big", 1)
    elseif user_opts.big_osc == true then 
        user_opts.big_osc = false 
        mp.osd_message("OSC size: always normal", 1)
    else
        user_opts.big_osc = 'auto'
        mp.osd_message("OSC size: auto", 1)
    end
    osc_init()
end
mp.add_key_binding(nil, 'toggle-big-osc', toggle_big_osc)


---------------------------------------------------------
local function toggle_ms_on_seekbar()
    state.tc_ms = not state.tc_ms
    request_init()
end
mp.add_key_binding(nil, 'toggle-ms-seekbar', toggle_ms_on_seekbar)


---------------------------------------------------------
local original_thumbnail_vertical_offset = thumbnailer_options.vertical_offset
local function toggle_seekbar_thumbnails_visibility()
    if thumbnailer_options.vertical_offset == original_thumbnail_vertical_offset then 
        thumbnailer_options.vertical_offset = -10000
    else
        thumbnailer_options.vertical_offset = original_thumbnail_vertical_offset
    end
end
mp.add_key_binding(nil, 'toggle-seekbar-thumbnails-visibility', toggle_seekbar_thumbnails_visibility)




---------------------------------------------------------
-- Gera hotkey til að birta tooltip eins og hover hover
local function toggle_seekbar_tooltip()
    if user_opts.persist_tooltip == 'none' then 
        user_opts.persist_tooltip = 'without_chap' 
    elseif user_opts.persist_tooltip == 'without_chap' then -- þetta er kinda stupid, líklega best að sleppa
        user_opts.persist_tooltip = 'with_chap' 
    else
        user_opts.persist_tooltip = 'none' 
    end
end
mp.add_key_binding(nil, 'toggle-seekbar-tooltip', toggle_seekbar_tooltip)

---------------------------------------------------------
-- Toggle að hovered kafli er highlightaður
local function toggle_chapter_highlight()
    if user_opts.highlight_chapters then 
        user_opts.highlight_chapters = false 
    else
        user_opts.highlight_chapters = true 
    end
end
mp.add_key_binding(nil, 'toggle-chapter-highlight', toggle_chapter_highlight)

--------------------------------------------------------------
-- Prenta current visibility
local function osc_print_visibility()
    mp.osd_message('OSC visibility set to: ' .. user_opts.visibility)
end
mp.add_key_binding(nil, 'osc-print-visibility', osc_print_visibility)
