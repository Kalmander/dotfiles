...
local SCRIPT_NAME = "seekbar_thumbnails"
...
thumbnailer_options['cache_directory'] = thumbnails_dir
...
function do_worker_job(state_json_string, frames_json_string)
    ...
    local generate_thumbnail_for_index = function(thumbnail_index)
        ...
        mp.commandv("script-message", "seekbar_thumbnails-progress", tostring(thumbnail_index))
        ...
        mp.commandv("script-message", "seekbar_thumbnails-ready", tostring(thumbnail_index), thumbnail_path)
    end
...
-- Job listener
mp.register_script_message("seekbar_thumbnails-job", do_worker_job)
...
local register_function = function()
    if mp.get_time() > register_timeout and register_timer then
        ...
        mp.commandv("script-message", "seekbar_thumbnails-worker", mp.get_script_name())
    end
end

register_timer = mp.add_periodic_timer(0.1, register_function)

mp.register_script_message("seekbar_thumbnails-slaved", function()
