...
--- TKJ EDIT START ---
-- Þessi trúður hafði enga leið til að stilla hvar loggið seivast svo ég bætti því við hér:
local log_dir = mp.command_native( {"expand-path", "~~/mpvClipboard.log" } )
--- TKJ EDIT END -----
...
local function copy()
	...
	if (filePath ~= nil) then
		...
		--mp.osd_message(copyLog)
		local copyLog = log_dir
...
function paste()
	...
	-- TKJ ef playlisti af youtube, senda nafnið á honum til playlistmanager
	if videoFile:match('youtu') and videoFile:match('playlist') then 
		mp.command('script-message playlistmanager-youtube-playlist-url ' .. videoFile)
		mp.command('script-message downloader-youtube-playlist-url ' .. videoFile)
	end
...
mp.add_key_binding(nil, 'copyCaps', copy)
mp.add_key_binding(nil, 'paste', paste)
mp.add_key_binding(nil, 'pasteCaps', paste)
mp.add_key_binding(nil, 'copy-path', copy_path)
mp.add_key_binding(nil, 'copy-pathCaps', copy_path)
mp.add_key_binding(nil, 'paste-playlist', paste_playlist)
mp.add_key_binding(nil, 'paste-playlistCaps', paste_playlist)
