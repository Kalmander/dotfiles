...
local playlist_dir = mp.command_native({"expand-path", "~~/playlists/"})
local settings  =  {
...
--END  OF  SAMPLE  REPLACE  ]=====]
    ...
    playlist_savepath  =  playlist_dir,
...
local function get_youtube_playlist_name(URL)
    args  =  {"yt-dlp",  "--ignore-config", "--get-filename",  "-o",  "'%(playlist_title)s'",  "--playlist-items",  "1",  URL}
...
--saves  the  current  playlist  into  an  m3u  file
function save_playlist()
    local length  =  mp.get_property_number('playlist-count',  0)
    if  length  ==  0  then  return end
    mp.osd_message('Saving  playlist...')
    ...
    if  youtube_playlist_url  ==  nil  then  
        savepath  =  utils.join_path(savepath,  datestring.."_playlist-size_"..length..".m3u")
    else
        youtube_playlist_name  =  get_youtube_playlist_name(youtube_playlist_url)
        savepath  =  utils.join_path(savepath,  youtube_playlist_name  ..  ".m3u")
    end


    savepath_info = utils.file_info(savepath)
    if savepath_info ~= nil then
        if savepath_info.is_file then 
    ...
    if not file then
        ...
        mp.osd_message("Playlist written to: "..savepath,  4)
        file:close()
    end
end
...
mp.register_script_message('playlistmanager-youtube-playlist-url',  set_playlist_url)
