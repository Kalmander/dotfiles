local mp = require 'mp'
package.path = mp.command_native( {"expand-path", "~~exe_dir/scripts" } ) .. "/?.lua;" .. package.path
local assdraw = require 'mp.assdraw'

local function overlay_maker()
    ...
    local overlay = { 
        ...
        list = {},
        list_style = [[{\q2\fs21\c&Hdddddd&}]],
        wrapper_style = [[{\c&bbbbbb&\fs16}]],
        top_wrapper_message = [[ item(s) above]],
        bottom_wrapper_message = [[ item(s) remaining]],
        ...
        update_ass = function(this)
            ...
            if start > 1 then this:append(this.wrapper_style..(start-1)..this.top_wrapper_message..'\\N\\N') end
    
            for i=start, finish do
                this:format_line(i, this.list[i])
            end
    
            if overflow then this:append('\\N'..this.wrapper_style..#this.list-finish..this.bottom_wrapper_message) end
            this.ass:update()
            this.bg_ass:update()
        end,
