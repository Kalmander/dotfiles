...
function detect_crop()
    ...
    mp.command(
        string.format(
            'no-osd vf pre @%s:cropdetect=limit=%s:round=%d:reset=0',
...
function apply_crop(meta)
    ...
    -- Apply crop.
    mp.command(
        string.format("no-osd vf pre @%s:lavfi-crop=w=%s:h=%s:x=%s:y=%s",
...
mp.add_key_binding(nil, "toggle_autocrop", on_toggle)
