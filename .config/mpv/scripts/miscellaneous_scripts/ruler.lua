...
(require 'mp.options').read_options(ruler_opts, 'ruler')

function ruler_split(input)
    local ret = {}
...
ruler_opts.confirm_bindings=ruler_split(ruler_opts.confirm_bindings)
ruler_opts.exit_bindings=ruler_split(ruler_opts.exit_bindings)
...
function ruler_get_video_dimensions()
...
function cursor_video_space()
    local dim = ruler_get_video_dimensions()
...
function ruler_refresh()
    ...
    if distinct then
        dot(line_start.screen, ruler_opts.dots_radius)
        line(line_start.screen, line_end.screen, ruler_opts.line_width)
        dot(line_end.screen, ruler_opts.dots_radius)
    else
        dot(line_start.screen, ruler_opts.dots_radius)
    end

    local line_info = function()
        if not ruler_opts.show_distance then return end
        a:new_event()
        a:append("{\\fs36}{\\bord1}")
        ...
        if ruler_opts.coordinates_space == "both" then
            a:append(string.format("image: %.1f\\Nscreen: %.1f", image, screen))
        elseif ruler_opts.coordinates_space == "image" then
            a:append(string.format("%.1f", image))
        elseif ruler_opts.coordinates_space == "window" then
            a:append(string.format("%.1f", screen))
        end
    end
    local dot_info = function(pos, opposite)
        if not ruler_opts.show_coordinates then return end
        a:new_event()
        a:append("{\\fs" .. ruler_opts.font_size .."}{\\bord1}")
    ...
    if distinct and ruler_opts.show_angles ~= "no" then
        local dist = 50
        ...
        line(line_start.screen, extended, math.max(0, ruler_opts.line_width-0.5))
        ...
        a:an(an)
        if ruler_opts.show_angles == "both" then
            a:append(string.format("%.2f\\N%.1f°", angle, angle / math.pi * 180))
        elseif ruler_opts.show_angles == "degrees" then
            a:append(string.format("%.1f°", angle / math.pi * 180))
        elseif ruler_opts.show_angles == "radians" then
...
function ruler_next()
    ...
    elseif state == 2 then
        ...
        if ruler_opts.clear_on_second_point_set then
            ruler_next()
        end
    else
        ruler_stop()
...
function ruler_stop()
    ...
    mp.unregister_idle(ruler_refresh)
    for _,key in ipairs(ruler_opts.confirm_bindings) do
