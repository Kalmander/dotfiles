...
-- Mínar audio stillingar
local function apply_audio_only_settings()
    mp.command('no-osd set keepaspect-window no')
end
local function remove_autio_only_settings()
    mp.command('no-osd set keepaspect-window yes')
end
mp.register_event('end-file', remove_autio_only_settings)
...
-- key bindings
-- cycle visualizer
local cycle_key = "v" -- ég disable-aði þetta neðst til að stilla frekar í input.conf
...
local function get_visualizer(name, quality)
    ...
    elseif name == "avectorscope" then
        ...
        -- The filter is used to measure the difference between channels of stereo audio stream. 
        -- A monoaural signal, consisting of identical left and right signal, results in straight 
        -- vertical line. Any stereo separation is visible as a deviation from this line, 
        -- creating a Lissajous figure. If the straight (or deviation from it) but horizontal 
        ...
        -- Red, green, blue og alpha contrast
        -- Leyfileg gildi: [0, 255]
        -- Sjálfvalið:
            -- rc = 40
            -- gc = 160
            -- bc = 80
            -- ac = 255
        local contrast = color_contrast_schemes.fire
        local rc = contrast.rc
        local gc = contrast.gc
        local bc = contrast.bc
        local ac = contrast.ac

        -- Red, green, blua og alpha fade
        -- Leyfileg gildi: [0, 255]
        -- Sjálfvalið:
            -- rf = 15
            -- gf = 10
            -- bf = 5
            -- af = 5
        local fade = color_fade_schemes.fire
        local rf = fade.rf
        local gf = fade.gf
        local bf = fade.bf
        local af = fade.af
        ...
        -- Leyfileg gildi:
            ...
            -- line: dregur línur á milli (punktarnir sjást samt enn sýnist mér og vantar stundum línur?)
        -- Default: dot
        local draw = 'dot'

        -- Skalinn á útslagi bylgjunnar
        -- Leyfileg gildi: 
            -- lin: Linear
            -- sqrt: Square root
            -- cbrt: Cube root
        ...
        -- Skipta á ásunnum
        -- Leyfileg gildi: 0 (ekki skipta) eða 1 (skipta)
        local swap = 0

        -- Spegla myndinni um ásanna
        -- Leyfileg gildi:
            -- none
            -- x  (speglar um x-ás)
            -- y  (speglar um y-ás)
            -- xy (speglar um báða ása)
        local mirror = 'none'

        local avectorscope_specs = 'avectorscope=' .. 
                'mode='   .. mode   .. ':' ..
                'size='   .. size   .. ':' ..
                'rate='   .. rate   .. ':' .. 
                'rc='     .. rc     .. ':' ..
...
local function select_visualizer(atrack, vtrack, albumart)
    ...
    elseif opts.mode == "novideo" then
        if vtrack == 0 then
            print(get_visualizer(opts.name, opts.quality))
            return get_visualizer(opts.name, opts.quality)
        end
        return ""
    end
