...
local hover_zone_ass = mp.create_osd_overlay('ass-events')
local assdraw = require 'mp.assdraw'

local _show_zones_info_by_default = false;
local _zones_key = "Ctrl+Alt+t"
local ZONE_THRESH_PERCENTAGE = 12;
-- -- sides get ZONE_THRESH_PERCENTAGE% each, mid gets (100-ZONE_THRESH_PERCENTAGE)%, same vertically
local VERT = {'top', 'middle', 'bottom'}
local HORZ = {'left', 'middle', 'right'}
local msg = mp.msg
local zone_highlighter_toggled = false
...
local function zone_highlighter()
    -- VERT = {'top', 'middle', 'bottom'}
    -- HORZ = {'left', 'middle', 'right'}
    local screenW, screenH = mp.get_osd_size()
    local mouseX, mouseY   = mp.get_mouse_pos()
    ...
    if (yZone ~= 'middle') or (xZone ~= 'middle') then 

        hover_zone_ass.res_x = screenW
        hover_zone_ass.res_y = screenH
        ...
        a:pos(0, 0)
        a:draw_start()
        a:rect_ccw(0, 0, screenW, threshY)
        a:rect_ccw(0, screenH-threshY, screenW, screenH)
        a:rect_ccw(0, threshY, threshX, screenH-threshY)
        a:rect_ccw(screenW-threshX, threshY, screenW, screenH-threshY)
        a:draw_stop()
        hover_zone_ass.data = a.text
        hover_zone_ass:update()
    else
        hover_zone_ass:remove()

    end
end

local function toggle_zone_highlighter()
    if zone_highlighter_toggled then 
        mp.unobserve_property(zone_highlighter)
        zone_highlighter_toggled = false
        hover_zone_ass:remove()
    else
        mp.observe_property('mouse-pos', 'native', zone_highlighter)
        zone_highlighter_toggled = true
    end
end

mp.add_key_binding(nil, 'toggle_zone_highlighter', toggle_zone_highlighter)