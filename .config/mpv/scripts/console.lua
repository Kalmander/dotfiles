...
local history = {}
local history_pos = 1
local log_buffer_blank_line_count = 40
local log_buffer = {}
-- Auka línur til að geta skrollað alveg upp
for i=1, log_buffer_blank_line_count do 
    log_buffer[i] = {style="", text=""}
end

local key_bindings = {}
local global_margin_y = 0
local display_start_pos = 0
local global_freeze = false
...
function log_add(style, text)
    ...
    if #log_buffer > 9999 + log_buffer_blank_line_count then
        table.remove(log_buffer, 1 + log_buffer_blank_line_count)
    elseif global_freeze == true then 
...
function update()
    ...
    local fontsize = opts.font_size
    local fullscreen_active = mp.get_property('fullscreen')
    ...
    local log_max_lines = math.ceil(screeny / fontsize)
    if log_max_lines < log_messages then
        log_messages = log_max_lines
    end
    if #log_buffer - log_messages < display_start_pos then 
        display_start_pos = #log_buffer - log_messages
    else

    display_start = #log_buffer - log_messages + 1 - display_start_pos
    display_end   = #log_buffer - display_start_pos
    end
    ...
    ass:append('{\\bord0}')
    ass:append('{\\shad0}')
    ass:append('{\\1c&' .. '#000000' .. '}')
    ass:append('{\\1a&' .. '280' .. '}')
    ass:pos(0, 0)
    ass:draw_start()
    ass:round_rect_cw(0, 0, 3840, 2160, 0)
    ass:draw_stop()
...
local skip_characters = {}
skip_characters[0x20] = true -- ' '
skip_characters[0x2d] = true -- '-'
skip_characters[0x2e] = true -- '.'
skip_characters[0x2f] = true -- '/'
skip_characters[0x3a] = true -- ':'
skip_characters[0x3b] = true -- ';'
skip_characters[0x3d] = true -- '='
skip_characters[0x5f] = true -- '_'
function prev_word_utf8(str, pos)
    if pos <= 1 then return pos end
    repeat
        pos = pos - 1
    until pos <= 1 or skip_characters[str:byte(pos)] ~= nil
    return pos
end
...
function handle_backspace_word()
    if cursor <= 1 then return end
    local prev = prev_word_utf8(line, cursor)
    line = line:sub(1, prev - 1) .. line:sub(cursor)
    cursor = prev
    update()
end
...
function handle_enter()
    ...
    local copy_line = line:match('^c%d+$')
    if help then
        help_command(help)
...
function move_log_display(amount)
    display_start_pos = display_start_pos + amount
    if display_start_pos < 0 then 
        display_start_pos = 0
    end
...
function jump_log_display()
    if display_start_pos ~= 0 then 
        display_start_pos = 0
        move_log_display(0)
    else 
        display_start_pos = #log_buffer
        move_log_display(-1)
        update()
    end
end
...
function adjust_font_size(amount)
    opts.font_size = opts.font_size + amount
    if opts.font_size < 10 then     -- min font size
        opts.font_size = 10
    elseif opts.font_size > 80 then -- max font size
        opts.font_size = 80
    end
    update()
end
...
function get_bindings()
    local bindings = {
        ...
        { 'f12',                function() set_active(false) end       },
        { 'enter',              handle_enter                           },
        { 'kp_enter',           handle_enter                           },
        { 'shift+enter',        function() handle_char_input('\n') end },
        { 'bs',                 handle_backspace                       },
        { 'shift+bs',           handle_backspace                       },
        { 'ctrl+bs',            handle_backspace_word                  },
        { 'del',                handle_del                             },
        { 'shift+del',          handle_del                             },
        { 'ins',                handle_ins                             },
        { 'shift+ins',          function() paste(false) end            },
        { 'mbtn_mid',           function() jump_log_display() end      },
        { 'left',               function() prev_char() end             },
        { 'right',              function() next_char() end             },
        { 'up',                 function() move_history(-1) end        },
        { 'wheel_up',           function() move_log_display(1) end     },
        { 'down',               function() move_history(1) end         },
        { 'wheel_down',         function() move_log_display(-1) end    },
        { 'ctrl+wheel_up',      function() adjust_font_size(1) end     },
        { 'ctrl+wheel_down',    function() adjust_font_size(-1) end    },
        { 'wheel_left',         function() move_log_display(15) end    },
        { 'wheel_right',        function() move_log_display(-15) end   },
        { 'ctrl+left',          prev_word                              },
...
-- PlayRes of the OSD will need to be 1ed.
...
mp.register_event('log-message', function(e)
    ...
    log_add(style,  string.format("%04d", message_number) .. ' [' .. e.prefix .. '] ' .. e.text)

    --nota þetta í download_video.lua, fokking stupid leið ég veit en datt ekkert betra í hug
    local match = string.match(e.text, [[echo Output name is ###"(.*)"###]])
    if (match ~= nil) then
        mp.commandv("script-message", 'ytdlp-output-name', e.text)
    end
end)
