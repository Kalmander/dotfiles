...
o = {
    ...
    videos = true,
    audio = true,
    reversed_dirs = {'videogamedunkey', 'Elliot Brooks'} -- in current implementation, this must be the end of the path to the directory
}
...
function alnumcomp_reversed(x, y)
    local xt, yt = clean_key(x), clean_key(y)
    for i = 1, math.min(#xt, #yt) do
        local xe, ye = xt[i], yt[i]
        if type(xe) == "string" then ye = tostring(ye)
...
local function find_and_add_entries()
    ...
    options.read_options(o) -- Reloada options td fyrir autoprofiles
    ...
    if file_path ~= nil then 
        local current_dir, _ = utils.split_path(file_path)
        if o.reversed_dirs == nil then o.reversed_dirs = {} end
        for _, revdir in pairs(o.reversed_dirs) do 
            local win_path = revdir .. '\\$'
            local lin_path = revdir .. '/$'
            if current_dir:match(win_path) or current_dir:match(lin_path) then 
                reverse_sort = true 
            end 
        end
    end
    ...
    if reverse_sort then 
        table.sort(files, alnumcomp_reversed)
    else
...
mp.add_key_binding(nil, 'autoload-folder', find_and_add_entries)
mp.observe_property('options/script-opts', 'string', find_and_add_entries) -- Refreshar playlista þegar script-opts hafa breyst (td út af autoprofile)
